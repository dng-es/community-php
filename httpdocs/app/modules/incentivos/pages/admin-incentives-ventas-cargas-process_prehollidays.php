		...
		if ($referencia_producto!=""){
			...
			if (count($producto)>0){
				if ($incentivos->insertIncentivesVenta($producto[0]['id_producto'], $cantidad_venta, $username_venta, $fecha_venta, $detalle, $tendencia)){
					...
					if ($incentivos->insertIncentivesProductosVentas($username_venta, $puntuacion_venta, $producto[0]['id_producto'], $fecha_venta, $detalle) and $puntuacion_venta > 0){
						users::sumarPuntos($username_venta, $puntuacion_venta, $detalle);
					}
				}
			}
			else $contador_ko ++;
