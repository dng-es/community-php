...
class usersCanalesController{
	public static function getListAction($reg = 0, $filter = ""){
		$users = new users();
		$find_reg = getFindReg();		
		if ($find_reg != '') $filter .= " AND canal_name LIKE '%".$find_reg."%' ";
		$filter .= " ORDER BY canal";
		
		$paginator_items = PaginatorPages($reg);	
		$total_reg = connection::countReg("canales", $filter); 
		return array('items' => $users->getCanales($filter.' LIMIT '.$paginator_items['inicio'].','.$reg),
	...
	public static function createAction(){
		if (isset($_POST['id_canal']) && $_POST['id_canal'] == ''){
			...
			$points_info = sanitizeInput($_POST['points_info']);
			$users = new users;
			if ($users->insertCanal($canal, $canal_name, $theme, $canal_lan, $points_info)){
				//crear foro asociado
				//$foro = new foro();
	...
	public static function updateAction(){
		if (isset($_POST['id_canal']) && $_POST['id_canal'] != ''){
			...
			$points_info = sanitizeInput($_POST['points_info']);
			$users = new users;
			if ($users->updateCanal($canal, $canal_name, $theme, $canal_lan, $points_info))
