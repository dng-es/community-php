...
class agenda{
	...
	public function insertActividad($titulo, $descripcion, $banner, $dateIni, $dateFin, $fichero, $tipo, $canal, $activo, $etiquetas){
		...
		//Carga del banner
		if (isset($banner['name']) && $banner['name'] != "") $banner = self::insertActividadFoto($banner);
		else $banner = '';

		//Formato fecha
		if (is_null($dateIni) || ($dateIni == '')) $date_ini = 'NULL';
		else{
			$date1 = str_replace('/', '-', trim(sanitizeInput($dateIni)));
			$date_ini = date('Y-m-d',strtotime($date1));
		}
		
		if (is_null($dateFin) || ($dateFin == '')) $date_fin = 'NULL';
		else{
